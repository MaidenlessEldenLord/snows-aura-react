...
export const Header = () => {
    ...
    const handleSubmit = (event) => {
        ...
        //const bagList = event.target.value; 
        event.target.reset(); 
        return null;
      }
  ...
  return (
    <header>      
      <nav className="bg-white drop-shadow-md border-b-2 border-gray-200 px-2 sm:px-4 py-2 dark:bg-blue-950 dark:border-b-1 dark:border-blue-950 dark:drop-shadow-md">
      <div className="flex flex-wrap justify-between items-center mx-auto">
          <Link to="" className="flex items-center">
              <img src='' className="mr-2 h-6 sm:h-9" alt=" " />
              <span className="self-center text-4xl font-semibold whitespace-nowrap dark:text-white">Snow's Aura</span>
          </Link>
          ...
          <button onClick={() => setDarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" data-toggle-dark="light" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
            ...
            <Link to="/cart" className="shoppingBag">
              <svg class="container h-10 w-10 fill-black dark:fill-white"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M160 112c0-35.3 28.7-64 64-64s64 28.7 64 64v48H160V112zm-48 48H48c-26.5 0-48 21.5-48 48V416c0 53 43 96 96 96H352c53 0 96-43 96-96V208c0-26.5-21.5-48-48-48H336V112C336 50.1 285.9 0 224 0S112 50.1 112 112v48zm24 48a24 24 0 1 1 0 48 24 24 0 1 1 0-48zm152 24a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z"/></svg>
            </Link>
          <div className={`${hidden ? "hidden" : ""} cartCounter font-bold text-black-700 rounded-full bg-blue-300 flex items-center justify-center font-mono h-5 w-5 mt-5`}>
            <span>{cartList.length}</span> 
          </div>
            <div className="hidden relative md:block">
              <div className="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                <svg className="w-6 h-6 ml-2 text-gray-500" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd"></path></svg>
                <span className="sr-only">Search icon</span>
              </div>
              <form className="container pl-3" onSubmit={handleSubmit}>
            ...
            <div className={`${hidden ? "hidden" : ""} justify-between items-center w-full md:flex md:w-auto md:order-1`} id="navbar-search">
              ...
              <ul className="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                ...
                <li>
                  <NavLink to="/candles/popular" className={({isActive}) => isActive ? activeClass : inActiveClass }>Popular</NavLink>
                </li>
                <li>
                  <NavLink to="/candles/top" className={({isActive}) => isActive ? activeClass : inActiveClass }>Top Rated</NavLink>
                </li>
                <li>
                  <NavLink to="/candles/upcoming" className={({isActive}) => isActive ? activeClass : inActiveClass }>Upcoming</NavLink>
                </li>
              </ul>
            </div>
        </div>
